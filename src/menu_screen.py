...
class MenuScreen(MDScreen):
    ...
    def on_pre_enter(self):
        ...
        data = {
            "profid": self.app.faculty_iD,
            "courseid": self.app.course,
            "weekid": self.weekid,
            "inf": "0",
        }
    ...
    def fetch_table(self, data):
        try:
            result = self.app.work.post(
                "http://timetable.msu.az/process.php",
                headers=HEADERS,
                data=data,
    ...
    def get_week_id(self, week=""):
        weeks = []
        data = {
            "prof_id": self.app.faculty_iD,
            "course_id": self.app.course,
            "inf": "0",
        }
        ...
        options_date = {
            option.get_text().replace(" ", ""): option["value"]
        ...
        for key, value in options_date.items():
            weeks.append(key)
            date_min, date_max = [
                datetime.strptime(date, "%d.%m.%Y") + timedelta(days=offset)
                for date, offset in zip(key.split("-"), [-1, 1])
            ]
    ...
    def table_to_days(self, table):  # function needs to be rewritten
        days = []
